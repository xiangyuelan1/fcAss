...
async def predict_with_community_model(
    ...
    try:
        checkpoint_meta = ModelCheckpoint.load_checkpoint_metadata(latest_task.id)
    ...
    if saved_feature_cols:
        ...
        if len(feature_cols) != len(saved_feature_cols):
            raise HTTPException(
                status_code=400,
                detail=f"模型需要{len(saved_feature_cols)}个特征，当前数据只有{len(feature_cols)}个可用，该模型可能不适用于此股票"
            )
    else:
    ...
    trained_stocks = user_model.stock_codes if user_model else []
    if feature_window > 1:
